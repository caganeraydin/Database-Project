...
@app.route('/')
def get_list_of_dentists():
    list_of_dentists = get_all_dentists()
    return render_template("appointments.html", List_of_all_dentists=list_of_dentists)
...
def insert_appointment():
    ...
    if not (get_invoice(invoice_id)):
        print("here2")
        flash("Invoice Id you entered does not exist, please try again.")
        return redirect(url_for('get_appointments'))
    if not (get_patient(patient_id)):
        print("here3")
        flash("Patient Id you entered does not exist, please try again.")
        return redirect(url_for('get_appointments'))
    if get_start_time(start_time) or get_start_time(end_time):
        print("here4")
        flash("The time you entered is not available, please choose another time.")
        return redirect(url_for('get_appointments'))
    else:
        print("here6")
        insert_appointment(appointment_id, invoice_id, patient_id, dentist_id, start_time, end_time, appointment_type,
